...
"""
import sys, json, os, threading, re, math, random, time, io
from pathlib import Path
from openai import OpenAI

# Force UTF-8 on Windows to avoid GBK encoding issues with Chinese text
...
def execute_single(question, model="ds-pro", region="auto", pathway_info=""):
    region_cn = {
        ...
        "visual_cortex": "Visual (vision)",
    }
    rname = region_cn.get(region, region)
    print(f"\n  {CC['W']}{'─'*60}{CC['R']}")
    print(f"  {CC['W']}BRAIN EXECUTION{CC['R']}")
    print(f"  {CC['D']}  Region:  {rname}{CC['R']}")
    print(f"  {CC['D']}  Pathway: {region} -> {model}{CC['R']}")
    if pathway_info:
        print(f"  {CC['D']}  Weight:  {pathway_info}{CC['R']}")
    print(f"  {CC['D']}  {'model':<16} {'status':<10} {'time':>7}  {'output':>7}{CC['R']}")
    t0 = time.time()
    print(f"  {CC['C']}> {MODELS[model][0]:<16} running     ...{CC['R']}")
    result = call(model, question)
    elapsed = time.time() - t0
    print(f"  {CC['G']}* {MODELS[model][0]:<16} done     {elapsed:.1f}s  {len(result)}c{CC['R']}")
...
def execute_collab(question, models):
    results = {}
    lock = threading.Lock()
    start_times = {m: time.time() for m in models}
    status = {m: "pending" for m in models}
    response_lens = {m: 0 for m in models}

    # Print parallel UI header
    print(f"\n  {CC['W']}{'─'*60}{CC['R']}")
    print(f"  {CC['W']}PARALLEL AI EXECUTION | {len(models)} models active{CC['R']}")
    print(f"  {CC['D']}{'model':<18} {'status':<12} {'time':>7}  {'output':>7}{CC['R']}")

    def _render():
        icons = {"running": f"{CC['C']}>", "done": f"{CC['G']}*", "failed": f"{CC['R']}X", "pending": "-"}
        for m in models:
            st = status.get(m, "pending")
            icon = icons.get(st, "?")
            elapsed = time.time() - start_times[m] if st != "pending" else 0
            tstr = f"{elapsed:.1f}s" if st != "pending" else "..."
            out = f"{response_lens[m]}c" if st == "done" else "..."
            print(f"  {icon} {MODELS.get(m,[m])[0]:<16} {st:<12} {tstr:>7}  {out:>7}{CC['R']}")

    def worker(mid):
        nonlocal results, status, response_lens
        try:
            status[mid] = "running"
            _render()
...
def tda_cache_question(question, dims, decision):
    ...
    try:
        ...
        TDA_CACHE_FILE.write_text(json.dumps(cache, ensure_ascii=True))
...
def main():
    ...
    # /neuro or neuro: or NEURO_MODE=1: force full parallel mode
    neuro_mode = os.environ.get('NEURO_MODE') == '1'
    if question.startswith('/neuro') or question.startswith('neuro:'):
        question = question.replace('/neuro', '', 1).replace('neuro:', '', 1).strip()
        neuro_mode = True
    ...
    # Neuro mode: always use at least 2 models + show full UI
    if neuro_mode:
        K = max(2, K)
        if scores["difficulty"] < 0.5:
            scores["difficulty"] = 0.5
        scores["collab_benefit"] = max(0.3, scores["collab_benefit"])
    ...
    if decision["action"] == "single":
        best_model = decision.get("model", "ds-pro")
        best_region = decision.get("region", "auto")
        answer = execute_single(question, best_model, region=best_region)
